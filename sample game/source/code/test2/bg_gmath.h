...
inline vec2 operator/(const vec2& a, float s) {
    float inv_s = static_cast<float>(1.0) / s;
    vec2 result = vec2::k_zero();
    for (int i = 0; i < vec2::size(); ++i) {
        result.e[i] = a.e[i] * inv_s;
    }
    return result;
}
...
inline bool vec2_equals_ep(const vec2& a, const vec2& b, float epsilon) {
    for (int i = 0; i < vec2::size(); ++i) {
        if (!(mf::equals_ep(a.e[i], b.e[i], epsilon))) {
            return false;
        }
    }
    return true;
...
inline float vec2_distance(const vec2& a, const vec2& b) {
    return std::sqrt((a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y));
}

inline vec2 vec2_scale(const vec2& a, float s) {
    vec2 result = vec2::k_zero();
    for (int i = 0; i < vec2::size(); ++i) {
        result.e[i] = a.e[i] * s;
    }
...
inline vec2 vec2_lerp(const vec2& a, const vec2& b, float s) {
    vec2 result = vec2::k_zero();
    for (int i = 0; i < vec2::size(); ++i) {
        result.e[i] = mf::lerp(a.e[i], b.e[i], s);
    }
    return result;
}
inline vec2 vec2_min(const vec2& a, const vec2& b) {
    vec2 result = vec2::k_zero();
    for (int i = 0; i < vec2::size(); ++i) {
        result.e[i] = mf::tmin(a.e[i], b.e[i]);
    }
    return result;
}
inline vec2 vec2_max(const vec2& a, const vec2& b) {
    vec2 result = vec2::k_zero();
    for (int i = 0; i < vec2::size(); ++i) {
        result.e[i] = mf::tmax(a.e[i], b.e[i]);
    }
    return result;
}
inline vec2 vec2_clamp(const vec2& a, const vec2& min, const vec2& max) {
    vec2 result = vec2::k_zero();
    for (int i = 0; i < vec2::size(); ++i) {
        result.e[i] = mf::clamp(a.e[i], min.e[i], max.e[i]);
    }
    return result;
}
...
inline float vec2_ccw(const vec2& a, const vec2& b) {
...
inline bool vec3_equals_ep(const vec3& a, const vec3& b, float epsilon) {
    for (int i = 0; i < vec3::size(); ++i) {
        if (!(mf::equals_ep(a.e[i], b.e[i], epsilon))) {
            return false;
        }
...
inline float vec3_length_sq(const vec3& a) {
    return (a.x * a.x + a.y * a.y + a.z * a.z);
}
inline float vec3_length(const vec3& a) {
    return std::sqrt(a.x * a.x + a.y * a.y + a.z * a.z);
}
inline float vec3_distance_sq(const vec3& a, const vec3& b) {
    return ((a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y) + (a.z - b.z) * (a.z - b.z));
}
inline float vec3_distance(const vec3& a, const vec3& b) {
    return std::sqrt((a.x - b.x) * (a.x - b.x) + (a.y - b.y) * (a.y - b.y) + (a.z - b.z) * (a.z - b.z));
}

inline vec3 vec3_scale(const vec3& a, float s) {
    vec3 result = vec3::k_zero();
    for (int i = 0; i < vec3::size(); ++i) {
        result.e[i] = a.e[i] * s;
    }
...
inline vec3 vec3_lerp(const vec3& a, const vec3& b, float s) {
    vec3 result = vec3::k_zero();
    for (int i = 0; i < vec3::size(); ++i) {
        result.e[i] = mf::lerp(a.e[i], b.e[i], s);
    }
...
// TOREMOVE:
// inline vec3 vec3_normalize_nochk(const vec3& a) {
//    vec3 result = vec3::k_zero();
...
inline std::optional<vec3> vec3_normalize(const vec3& a) {
    vec3 result = vec3::k_zero();
    float length = vec3_length(a);
...
inline vec3 vec3_ma(const vec3& a, const vec3& b, float s) {
    vec3 result = vec3::k_zero();
    for (int i = 0; i < vec3::size(); ++i) {
        result.e[i] = a.e[i] + b.e[i] * s;
    }
    return result;
}
...
inline bool vec4_equals_ep(const vec4& a, const vec4& b, float epsilon) {
    for (int i = 0; i < vec4::size(); ++i) {
        if (!(mf::equals_ep(a.e[i], b.e[i], epsilon))) {
            return false;
        }
...
inline vec4 vec4_lerp(const vec4& a, const vec4& b, float s) {
    vec4 result = vec4::k_zero();
    for (int i = 0; i < vec4::size(); ++i) {
        result.e[i] = mf::lerp(a.e[i], b.e[i], s);
    }
...
inline vec4 vec4_clamp(const vec4& a, const vec4& min, const vec4& max) {
    vec4 result = vec4::k_zero();
    for (int i = 0; i < vec4::size(); ++i) {
        result.e[i] = mf::clamp(a.e[i], min.e[i], max.e[i]);
    }
...
//inline mat4 operator * (const mat4& a, const mat4& b) { mat4 result = mat4::k_zero(); for (int i=0; i < mat4::size(); ++i) { result.e[i] = a.e[i] * b.e[i]; } return result; }
//inline mat4 operator / (const mat4& a, const mat4& b) { mat4 result = mat4::k_zero(); for (int i=0; i < mat4::size(); ++i) { result.e[i] = a.e[i] / b.e[i]; } return result; }
...
// Same as mat4_mul
inline mat4 mat4_mul_loop(const mat4& ma, const mat4& mb) {
    mat4 out = mat4::k_zero();
    for (int i = 0; i < mat4::dim(); i++) {
        for (int j = 0; j < mat4::dim(); j++) {
            out.r[i][j] = ma.r[i][0] * mb.r[0][j] + ma.r[i][1] * mb.r[1][j] + ma.r[i][2] * mb.r[2][j] + ma.r[i][3] * mb.r[3][j];
        }
    }
    return out;
}
...
inline mat4 mat4_mul_cw(const mat4& a, const mat4& b) {
    mat4 result = mat4::k_zero();
    for (int i = 0; i < mat4::size(); ++i) {
        result.e[i] = a.e[i] * b.e[i];
    }
...
inline mat4 mat4_div_cw(const mat4& a, const mat4& b) {
    mat4 result = mat4::k_zero();
    for (int i = 0; i < mat4::size(); ++i) {
        result.e[i] = a.e[i] / b.e[i];
    }
...
inline bool mat4_equals_ep(const mat4& a, const mat4& b, float epsilon) {
    for (int i = 0; i < mat4::size(); ++i) {
        if (!(mf::equals_ep(a.e[i], b.e[i], epsilon))) {
            return false;
        }
...
inline mat4 mat4_lerp(const mat4& a, const mat4& b, float s) {
    mat4 result = mat4::k_zero();
    for (int i = 0; i < mat4::size(); ++i) {
        result.e[i] = mf::lerp(a.e[i], b.e[i], s);
    }
...
inline vec4 operator*(const vec4& va, const mat4& ma) {
    return vec4(
        va.e[0] * ma.r[0][0] + va.e[1] * ma.r[1][0] + va.e[2] * ma.r[2][0] + va.e[3] * ma.r[3][0],
        va.e[0] * ma.r[0][1] + va.e[1] * ma.r[1][1] + va.e[2] * ma.r[2][1] + va.e[3] * ma.r[3][1],
        va.e[0] * ma.r[0][2] + va.e[1] * ma.r[1][2] + va.e[2] * ma.r[2][2] + va.e[3] * ma.r[3][2],
        va.e[0] * ma.r[0][3] + va.e[1] * ma.r[1][3] + va.e[2] * ma.r[2][3] + va.e[3] * ma.r[3][3]);
}
inline vec4 operator*(const mat4& ma, const vec4& va) {
    return vec4(
        ma.r[0][0] * va.e[0] + ma.r[0][1] * va.e[1] + ma.r[0][2] * va.e[2] + ma.r[0][3] * va.e[3],
        ma.r[1][0] * va.e[0] + ma.r[1][1] * va.e[1] + ma.r[1][2] * va.e[2] + ma.r[1][3] * va.e[3],
        ma.r[2][0] * va.e[0] + ma.r[2][1] * va.e[1] + ma.r[2][2] * va.e[2] + ma.r[2][3] * va.e[3],
        ma.r[3][0] * va.e[0] + ma.r[3][1] * va.e[1] + ma.r[3][2] * va.e[2] + ma.r[3][3] * va.e[3]);
}
...
// inline color4 operator / (const color4& a, const color4& b) { color4 result = color4::k_zero(); for (int i=0; i < color4::size(); ++i) { result.e[i] = a.e[i] / b.e[i]; } return result; }

inline color4 operator+(const color4& a, float s) {
    color4 result = color4::k_zero();
    for (int i = 0; i < color4::size(); ++i) {
        result.e[i] = a.e[i] + s;
    }
...
inline color4 operator+(float s, const color4& a) {
    color4 result = color4::k_zero();
    for (int i = 0; i < color4::size(); ++i) {
        result.e[i] = a.e[i] + s;
    }
...
inline bool operator==(const color4& a, const color4& b) {
    for (int i = 0; i < color4::size(); ++i) {
        if (!(a.e[i] == b.e[i])) {
            return false;
        }
...
inline bool color4_equals_ep(const color4& a, const color4& b, float epsilon) {
    for (int i = 0; i < color4::size(); ++i) {
        if (!(mf::equals_ep(a.e[i], b.e[i], epsilon))) {
            return false;
        }
...
// inline bool operator < (const color4& a, const color4& b)	{ for (int i=0; i < color4::size(); ++i) { if (!(a.e[i] < b.e[i])) { return false; } }	return true; }
// inline bool operator > (const color4& a, const color4& b)	{ for (int i=0; i < color4::size(); ++i) { if (!(a.e[i] > b.e[i])) { return false; } }	return true; }
// inline bool operator <= (const color4& a, const color4& b)	{ for (int i=0; i < color4::size(); ++i) { if (!(a.e[i] <= b.e[i])) { return false; } }	return true; }
// inline bool operator >= (const color4& a, const color4& b)	{ for (int i=0; i < color4::size(); ++i) { if (!(a.e[i] >= b.e[i])) { return false; } }	return true; }

inline color4 color4_scale(const color4& a, float s) {
    color4 result = color4::k_zero();
    for (int i = 0; i < color4::size(); ++i) {
        result.e[i] = a.e[i] * s;
    }
...
inline color4 color4_lerp(const color4& a, const color4& b, float s) {
    color4 result = color4::k_zero();
    for (int i = 0; i < color4::size(); ++i) {
        result.e[i] = mf::lerp(a.e[i], b.e[i], s);
    }
...
// Unpacks four uint8_t components from uint32_t and assign to uint32_t.
...
inline uint32_t ftouc(float x) {
    ...
    return static_cast<uint32_t>(x >= 1.0f ? UCHAR_MAX : x <= 0.0f ? 0x00
                                                                   : static_cast<uint32_t>(x * 255.0f));
}
...
inline uint32_t ftouc_d3dx9(float x) {
    ...
    return static_cast<uint32_t>(x >= 1.0f ? UCHAR_MAX : x <= 0.0f ? 0x00
                                                                   : static_cast<uint32_t>(x * 255.0f + 0.5f));
}
...
// Unpacks four uint8_t components from uint32_t and assign to floats.
...
inline color4 argb_to_color4(uint32_t uc) {
    ...
    // result.r = f * (float) (uint8_t) (uc >> 16);
    // result.g = f * (float) (uint8_t) (uc >>  8);
    // result.b = f * (float) (uint8_t) (uc >>  0);
    // result.a = f * (float) (uint8_t) (uc >> 24);

    unpackf_u32(uc, result.a, result.r, result.g, result.b);
    return result;
}

inline color4 abgr_to_color4(uint32_t uc) {
    color4 result = color4::k_zero();
    unpackf_u32(uc, result.a, result.b, result.g, result.r);
    return result;
}

inline color4 rgba_to_color4(uint32_t uc) {
    color4 result = color4::k_zero();
    unpackf_u32(uc, result.r, result.g, result.b, result.a);
    return result;
}
...
inline color4 color4_adjust_saturation(const color4& a, float s) {
    ...
    result.r = grey + s * (a.r - grey);
    result.g = grey + s * (a.g - grey);
    result.b = grey + s * (a.b - grey);
    result.a = a.a;
...
inline color4 color4_adjust_contrast(const color4& a, float s) {
    ...
    result.r = factor + s * (a.r - factor);
    result.g = factor + s * (a.g - factor);
    result.b = factor + s * (a.b - factor);
    result.a = a.a;
...
// inline quat operator * (const quat& a, const quat& b) { quat result = quat::k_zero(); for (int i=0; i < quat::size(); ++i) { result.e[i] = a.e[i] * b.e[i]; } return result; }
// inline quat operator / (const quat& a, const quat& b) { quat result = quat::k_zero(); for (int i=0; i < quat::size(); ++i) { result.e[i] = a.e[i] / b.e[i]; } return result; }

// Quaternion multiplication. The result represents the rotation b followed by the rotation a. (result = b * a)
// quat_mul
inline quat operator*(const quat& a, const quat& b) {
    quat result = quat::k_zero();
...
inline quat operator+(float s, const quat& a) {
    quat result = quat::k_zero();
    for (int i = 0; i < quat::size(); ++i) {
        result.e[i] = a.e[i] + s;
    }
...
inline bool quat_equals_ep(const quat& a, const quat& b, float epsilon) {
    for (int i = 0; i < quat::size(); ++i) {
        if (!(mf::equals_ep(a.e[i], b.e[i], epsilon))) {
            return false;
        }
...
//inline bool operator<(const quat& a, const quat& b)     { for (int i = 0; i < quat::size(); ++i) { if (!(a.e[i] < b.e[i])) { return false; } } return true; }
//inline bool operator>(const quat& a, const quat& b)     { for (int i = 0; i < quat::size(); ++i) { if (!(a.e[i] > b.e[i])) { return false; } } return true; }
//inline bool operator<=(const quat& a, const quat& b)    { for (int i = 0; i < quat::size(); ++i) { if (!(a.e[i] <= b.e[i])) { return false; } } return true; }
//inline bool operator>=(const quat& a, const quat& b)    { for (int i = 0; i < quat::size(); ++i) { if (!(a.e[i] >= b.e[i])) { return false; } } return true; }
...
inline float quat_length_sq(const quat& a) {
    return (a.x * a.x + a.y * a.y + a.z * a.z + a.w * a.w);
}
inline float quat_length(const quat& a) {
    return std::sqrt(a.x * a.x + a.y * a.y + a.z * a.z + a.w * a.w);
}
...
inline quat quat_ma(const quat& a, const quat& b, float s) {
    quat result = quat::k_zero();
    for (int i = 0; i < quat::size(); ++i) {
        result.e[i] = a.e[i] + b.e[i] * s;
    }
...
inline std::optional<mat4> mat4_inverse(const mat4& ma) {
    ...
    result.r[0][0] = c11 * inv_det;
    result.r[1][0] = c12 * inv_det;
    result.r[2][0] = c13 * inv_det;
    result.r[3][0] = c14 * inv_det;

    // Second Row |c21, c22, c23, c24|, transposed & divided by det
    result.r[0][1] = -(b * kp_lo - c * jp_ln + d * jo_kn) * inv_det;
    result.r[1][1] = +(a * kp_lo - c * ip_lm + d * io_km) * inv_det;
    result.r[2][1] = -(a * jp_ln - b * ip_lm + d * in_jm) * inv_det;
    result.r[3][1] = +(a * jo_kn - b * io_km + c * in_jm) * inv_det;
    ...
    result.r[0][2] = +(b * gp_ho - c * fp_hn + d * fo_gn) * inv_det;
    result.r[1][2] = -(a * gp_ho - c * ep_hm + d * eo_gm) * inv_det;
    result.r[2][2] = +(a * fp_hn - b * ep_hm + d * en_fm) * inv_det;
    result.r[3][2] = -(a * fo_gn - b * eo_gm + c * en_fm) * inv_det;
    ...
    result.r[0][3] = -(b * gl_hk - c * fl_hj + d * fk_gj) * inv_det;
    result.r[1][3] = +(a * gl_hk - c * el_hi + d * ek_gi) * inv_det;
    result.r[2][3] = -(a * fl_hj - b * el_hi + d * ej_fi) * inv_det;
    result.r[3][3] = +(a * fk_gj - b * ek_gi + c * ej_fi) * inv_det;
...
inline mat4 mat4_rotation_axis(const vec3& axis, float angle) {    
    vec3 nv = vec3_normalize(axis).value();
...
//wnm
inline mat4 mat4_look_at_lh(const vec3& eye, const vec3& at, const vec3& up) {
    vec3 zaxis = vec3_normalize(at - eye).value();
    vec3 xaxis = vec3_normalize(vec3_cross(up, zaxis)).value();
...
//wnm
inline mat4 mat4_look_at_rh(const vec3& eye, const vec3& at, const vec3& up) {
    vec3 zaxis = vec3_normalize(eye - at).value();
    vec3 xaxis = vec3_normalize(vec3_cross(up, zaxis)).value();
    vec3 yaxis = vec3_cross(zaxis, xaxis);

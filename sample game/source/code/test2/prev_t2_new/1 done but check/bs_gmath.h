...
template<class T> inline const T k_identity;
template<class T> inline const T k_one;
//template<class T> inline const T k_origin;
//template<class T> inline const T k_unit;


//---------------------------------------------------
// vec2
//---------------------------------------------------
template<> inline const vec2 k_zero<vec2> = vec2(0.0);
//template<> inline const vec2 k_identity<vec2> = vec2(1.0);
//template<> inline const vec2 k_origin<vec2> = vec2(0.0);
//template<> inline const vec2 k_unit<vec2> = vec2(1.0);

inline vec2 operator - (const vec2& a) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = -a[i]; } return out; }

inline vec2 operator + (const vec2& a, const vec2& b) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] + b[i]; } return out; }
inline vec2 operator - (const vec2& a, const vec2& b) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] - b[i]; } return out; }
inline vec2 operator * (const vec2& a, const vec2& b) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] * b[i]; } return out; }
inline vec2 operator / (const vec2& a, const vec2& b) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] / b[i]; } return out; }

inline vec2 operator + (const vec2& a, float s) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] + s; } return out; }
inline vec2 operator - (const vec2& a, float s) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] - s; } return out; }
inline vec2 operator * (const vec2& a, float s) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] * s; } return out; }
inline vec2 operator / (const vec2& a, float s) { float inv_s = static_cast<float>(1.0)/s; vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] * inv_s; } return out; }

inline vec2 operator + (float s, const vec2& a) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] + s; } return out; }
inline vec2 operator - (float s, const vec2& a) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] - s; } return out; }
inline vec2 operator * (float s, const vec2& a) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] * s; } return out; }
inline vec2 operator / (float s, const vec2& a) { float inv_s = static_cast<float>(1.0)/s; vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] * inv_s; } return out; }
...
inline float vec2_dot(const vec2& a, const vec2& b) { float sum = 0.0;  for (int i=0; i < vec2::size(); ++i) { sum += a[i] * b[i]; }    return sum; }
inline float vec2_length_sq(const vec2& a) { float sum = 0.0;  for (int i=0; i < vec2::size(); ++i) { sum += a[i] * a[i]; }    return sum; }
inline float vec2_length(const vec2& a) { float sum = 0.0;  for (int i=0; i < vec2::size(); ++i) { sum += a[i] * a[i]; }    return sqrt(sum); }
inline float vec2_distance_sq(const vec2& a, const vec2& b) { float sum = 0.0;  for (int i=0; i < vec2::size(); ++i) { sum += (a[i] - b[i])*(a[i] - b[i]); }    return sum; }
inline float vec2_distance(const vec2& a, const vec2& b) { float sum = 0.0;  for (int i=0; i < vec2::size(); ++i) { sum += (a[i] - b[i])*(a[i] - b[i]); }    return sqrt(sum); }

inline vec2 vec2_scale(const vec2& a, float s) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = a[i] * s; } return out; }
inline vec2 vec2_lerp(const vec2& a, const vec2& b, float s) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = mf::lerp(a[i], b[i], s); } return out; }
inline vec2 vec2_min(const vec2& a, const vec2& b) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = sdf::tmin(a[i], b[i]); } return out; }
inline vec2 vec2_max(const vec2& a, const vec2& b) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = sdf::tmax(a[i], b[i]); } return out; }
inline vec2 vec2_clamp(const vec2& a, const vec2& min, const vec2& max) { vec2 out = k_zero<vec2>; for (int i=0; i < vec2::size(); ++i) { out[i] = sdf::clamp(a[i], min[i], max[i]); } return out; }

inline vec2 vec2_normalize(const vec2& a) { 
    vec2 out = k_zero<vec2>; 
...
// vec3
//---------------------------------------------------
template<> inline const vec3 k_zero<vec3> = vec3(0.0);
template<> inline const vec3 k_one<vec3> = vec3(1.0);

inline vec3 operator - (const vec3& a) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = -a[i]; } return out; }

inline vec3 operator + (const vec3& a, const vec3& b) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] + b[i]; } return out; }
inline vec3 operator - (const vec3& a, const vec3& b) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] - b[i]; } return out; }
inline vec3 operator * (const vec3& a, const vec3& b) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] * b[i]; } return out; }
inline vec3 operator / (const vec3& a, const vec3& b) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] / b[i]; } return out; }

inline vec3 operator + (const vec3& a, float s) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] + s; } return out; }
inline vec3 operator - (const vec3& a, float s) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] - s; } return out; }
inline vec3 operator * (const vec3& a, float s) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] * s; } return out; }
inline vec3 operator / (const vec3& a, float s) { float inv_s = static_cast<float>(1.0)/s; vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] * inv_s; } return out; }

inline vec3 operator + (float s, const vec3& a) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] + s; } return out; }
inline vec3 operator - (float s, const vec3& a) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] - s; } return out; }
inline vec3 operator * (float s, const vec3& a) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] * s; } return out; }
inline vec3 operator / (float s, const vec3& a) { float inv_s = static_cast<float>(1.0)/s; vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] * inv_s; } return out; }
...
inline float vec3_dot(const vec3& a, const vec3& b) { float sum = 0.0;  for (int i=0; i < vec3::size(); ++i) { sum += a[i] * b[i]; }    return sum; }
inline float vec3_length_sq(const vec3& a) { float sum = 0.0;  for (int i=0; i < vec3::size(); ++i) { sum += a[i] * a[i]; }    return sum; }
inline float vec3_length(const vec3& a) { float sum = 0.0;  for (int i=0; i < vec3::size(); ++i) { sum += a[i] * a[i]; }    return sqrt(sum); }
inline float vec3_distance_sq(const vec3& a, const vec3& b) { float sum = 0.0;  for (int i=0; i < vec3::size(); ++i) { sum += (a[i] - b[i])*(a[i] - b[i]); }    return sum; }
inline float vec3_distance(const vec3& a, const vec3& b) { float sum = 0.0;  for (int i=0; i < vec3::size(); ++i) { sum += (a[i] - b[i])*(a[i] - b[i]); }    return sqrt(sum); }

inline vec3 vec3_scale(const vec3& a, float s) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = a[i] * s; } return out; }
inline vec3 vec3_lerp(const vec3& a, const vec3& b, float s) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = mf::lerp(a[i], b[i], s); } return out; }
inline vec3 vec3_min(const vec3& a, const vec3& b) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = sdf::tmin(a[i], b[i]); } return out; }
inline vec3 vec3_max(const vec3& a, const vec3& b) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = sdf::tmax(a[i], b[i]); } return out; }
inline vec3 vec3_clamp(const vec3& a, const vec3& min, const vec3& max) { vec3 out = k_zero<vec3>; for (int i=0; i < vec3::size(); ++i) { out[i] = sdf::clamp(a[i], min[i], max[i]); } return out; }
...
// vec4
//---------------------------------------------------
template<> inline const vec4 k_zero<vec4> = vec4(0.0);

inline vec4 operator - (const vec4& a) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = -a[i]; } return out; }

inline vec4 operator + (const vec4& a, const vec4& b) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] + b[i]; } return out; }
inline vec4 operator - (const vec4& a, const vec4& b) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] - b[i]; } return out; }
inline vec4 operator * (const vec4& a, const vec4& b) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] * b[i]; } return out; }
inline vec4 operator / (const vec4& a, const vec4& b) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] / b[i]; } return out; }

inline vec4 operator + (const vec4& a, float s) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] + s; } return out; }
inline vec4 operator - (const vec4& a, float s) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] - s; } return out; }
inline vec4 operator * (const vec4& a, float s) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] * s; } return out; }
inline vec4 operator / (const vec4& a, float s) { float inv_s = static_cast<float>(1.0)/s; vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] * inv_s; } return out; }

inline vec4 operator + (float s, const vec4& a) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] + s; } return out; }
inline vec4 operator - (float s, const vec4& a) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] - s; } return out; }
inline vec4 operator * (float s, const vec4& a) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] * s; } return out; }
inline vec4 operator / (float s, const vec4& a) { float inv_s = static_cast<float>(1.0)/s; vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] * inv_s; } return out; }
...
inline vec4 vec4_scale(const vec4& a, float s) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = a[i] * s; } return out; }
inline vec4 vec4_lerp(const vec4& a, const vec4& b, float s) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = mf::lerp(a[i], b[i], s); } return out; }
inline vec4 vec4_min(const vec4& a, const vec4& b) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = sdf::tmin(a[i], b[i]); } return out; }
inline vec4 vec4_max(const vec4& a, const vec4& b) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = sdf::tmax(a[i], b[i]); } return out; }
inline vec4 vec4_clamp(const vec4& a, const vec4& min, const vec4& max) { vec4 out = k_zero<vec4>; for (int i=0; i < vec4::size(); ++i) { out[i] = sdf::clamp(a[i], min[i], max[i]); } return out; }
...
// mat4
//---------------------------------------------------
template<> inline const mat4 k_zero<mat4> = mat4(0.0);
template<> inline const mat4 k_identity<mat4> = mat4(1.0);

inline mat4 operator - (const mat4& a) { mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = -a[i]; } return out; }

inline mat4 operator + (const mat4& a, const mat4& b) { mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = a[i] + b[i]; } return out; }
inline mat4 operator - (const mat4& a, const mat4& b) { mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = a[i] - b[i]; } return out; }
...
inline mat4 mat4_mul_cw(const mat4& a, const mat4& b) { mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = a[i] * b[i]; } return out; }
inline mat4 mat4_div_cw(const mat4& a, const mat4& b) { mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = a[i] / b[i]; } return out; }

inline mat4 operator + (const mat4& a, float s) { mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = a[i] + s; } return out; }
inline mat4 operator - (const mat4& a, float s) { mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = a[i] - s; } return out; }
inline mat4 operator * (const mat4& a, float s) { mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = a[i] * s; } return out; }
inline mat4 operator / (const mat4& a, float s) { float inv_s = static_cast<float>(1.0)/s; mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = a[i] * inv_s; } return out; }

inline mat4 operator + (float s, const mat4& a) { mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = a[i] + s; } return out; }
inline mat4 operator - (float s, const mat4& a) { mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = a[i] - s; } return out; }
inline mat4 operator * (float s, const mat4& a) { mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = a[i] * s; } return out; }
inline mat4 operator / (float s, const mat4& a) { float inv_s = static_cast<float>(1.0)/s; mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = a[i] * inv_s; } return out; }
...
inline mat4 mat4_lerp(const mat4& a, const mat4& b, float s) { mat4 out = k_zero<mat4>; for (int i=0; i < mat4::size(); ++i) { out[i] = mf::lerp(a[i], b[i], s); } return out; }
...
// color4
//---------------------------------------------------
template<> inline const color4 k_zero<color4> = color4(0.0);

inline color4 operator - (const color4& a) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = -a[i]; } return out; }

inline color4 operator + (const color4& a, const color4& b) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] + b[i]; } return out; }
inline color4 operator - (const color4& a, const color4& b) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] - b[i]; } return out; }
inline color4 operator * (const color4& a, const color4& b) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] * b[i]; } return out; }
//inline color4 operator / (const color4& a, const color4& b) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] / b[i]; } return out; }

inline color4 operator + (const color4& a, float s) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] + s; } return out; }
inline color4 operator - (const color4& a, float s) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] - s; } return out; }
inline color4 operator * (const color4& a, float s) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] * s; } return out; }
inline color4 operator / (const color4& a, float s) { float inv_s = static_cast<float>(1.0)/s; color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] * inv_s; } return out; }

inline color4 operator + (float s, const color4& a) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] + s; } return out; }
inline color4 operator - (float s, const color4& a) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] - s; } return out; }
inline color4 operator * (float s, const color4& a) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] * s; } return out; }
inline color4 operator / (float s, const color4& a) { float inv_s = static_cast<float>(1.0)/s; color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] * inv_s; } return out; }
...
inline color4 color4_scale(const color4& a, float s) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = a[i] * s; } return out; }
inline color4 color4_lerp(const color4& a, const color4& b, float s) { color4 out = k_zero<color4>; for (int i=0; i < color4::size(); ++i) { out[i] = mf::lerp(a[i], b[i], s); } return out; }
...
inline uint packf_u32_d3dx9(float a, float b, float c, float d) {
...
inline uint color4_to_argb_d3dx9(const color4& a) {
	...
	return packf_u32_d3dx9(a.a, a.r, a.g ,a.b);
}
...
// quat
//---------------------------------------------------
template<> inline const quat k_zero<quat> = quat(0.0);
template<> inline const quat k_identity<quat> = quat(0, 0, 0, 1);
...
inline float quat_dot(const quat& a, const quat& b) { float sum = 0.0;  for (int i=0; i < quat::size(); ++i) { sum += a[i] * b[i]; }    return sum; }
// Length squared, or "norm"
inline float quat_length_sq(const quat& a) { float sum = 0.0;  for (int i=0; i < quat::size(); ++i) { sum += a[i] * a[i]; }    return sum; }
inline float quat_length(const quat& a) { float sum = 0.0;  for (int i=0; i < quat::size(); ++i) { sum += a[i] * a[i]; }    return sqrt(sum); }

inline quat quat_normalize(const vec4& a) { 
    quat out = k_zero<quat>; 
	float length = vec4_length(a);
...
inline quat quat_mul_angles(const quat& a, const quat& b, float s) { quat out = k_zero<quat>; for (int i=0; i < quat::size(); ++i) { out[i] = a[i] + s*b[i]; } return out; }


//---------------------------------------------------
// plane
//---------------------------------------------------
template<> inline const plane k_zero<plane> = plane(0.0);

...
def process_vk_ext(fin_h, fout_h, fout_cpp, profile_type):
    ...
    for line in Lines:        
        ...
        if (is_version_line):
            ...
            vk_core_init_func_versions_list_cpp.append("\nvoid vkfnlib_init_" + version_name + "(PFN_loadfunc load) {\n")
            vk_core_init_func_list_cpp.append("vkfnlib_init_" + version_name + "(load);")     
        ...
        if 'VKAPI_ATTR' in line and 'VKAPI_CALL' in line:
            ...
            for i in range(0, len(words)):
                if (words[i] == 'VKAPI_CALL'):
                    ...
                    # cpp
                    func_init_cpp = func_name + " = (" + func_name_ptr + ")" + vk_loader_func + '("' + func_name + '");'
    ...
    if (profile_type == "vkwin32"):
        ...
        print("void " + loader_internal_init + "(PFN_loadfunc load) {")
    ...
    print("\n\nvoid " + loader_init_func_name + "(PFN_loadfunc load) {")

    if (profile_type == "vkcore"):
        for i in vk_core_init_func_list_cpp:
            print("\t" + i) 
    else:
        print("\t" + loader_internal_init + "(load);") 
    ...
    sys.stdout = fout_h    
    print("\n\n// Loader Function")
    print("void " + loader_init_func_name + "(PFN_loadfunc load);")

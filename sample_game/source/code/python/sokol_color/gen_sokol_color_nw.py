...
header = open("base_gmath_colors.h", "w")
...
def add_documentation(color):
    documentation = "// {name} {{ R:{r}, G:{g}, B:{b}, A:{a} }}, {{ {hex_code} }} \n"
    hex_color = "0x{0:08X}".format(color[1])
    rgba = unpack_rgba(color[1])
    header.write(documentation.format(
        name = color[0], r = rgba[0], g = rgba[1], b = rgba[2], a = rgba[3], hex_code = hex_color))

for color in colors:
    add_documentation(color)
    init_color = color[0].lower().replace(" ", "_")
    init_color_definition = "inline const color4 {name} = {{ {r}f, {g}f, {b}f, {a}f }};\n"
    rgba = unpack_rgba(color[1])
    r = rgba[0] / 255
    g = rgba[1] / 255
    b = rgba[2] / 255
...
#    #add_documentation(color)
#    hex_color = "0x{0:08X}".format(color[1])
#    packed_color = color[0].lower().replace(" ", "_") #+ "_u32"
#    packed_color_definition = "inline constexpr uint {name} = {rgba};\n"

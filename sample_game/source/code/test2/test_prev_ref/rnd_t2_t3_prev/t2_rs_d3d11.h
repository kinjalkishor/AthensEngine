...
inline void d3d11_create_input_element_desc(sdf::span<rs_input_element_desc> iedesc, sdr::vector<D3D11_INPUT_ELEMENT_DESC>& input_elems) {
	...
	for_range (i, 0, iedesc.size()) {
		...
		input_elems.push_back({iedesc[i].semantic_name, 0, vformat, 0, scast<UINT>(curr_aligned_byte_offset), D3D11_INPUT_PER_VERTEX_DATA, 0});
...
public:
	...
	void create_vertex_buffer(UINT vb_size_bytes, const void* sys_mem_data, ID3D11Buffer*& vb) {
	...
	void create_index_buffer(UINT ib_size_bytes, const void* sys_mem_data, ID3D11Buffer*& ib) {
	...
	void create_constant_buffer(UINT cb_size_bytes, ID3D11Buffer*& buffer) {
	...
	void upload_constant_buffer(ID3D11Buffer* constant_buffers, UINT num_buffers, const void* src_data) {
	...
	void create_texture(const char* tex_file_name, ID3D11ShaderResourceView*& tex) {
		...
		if (tex_is_dds) {
			HRESULT hr = DirectX::CreateDDSTextureFromFile(m_device, tex_file_namew.c_str(), nullptr,  &tex);
			...
			HRESULT hr = DirectX::CreateWICTextureFromFile(m_device, tex_file_namew.c_str(), nullptr,  &tex);
			if (FAILED(hr)) { eprintfln("DirectX::CreateWICTextureFromFile: Cannot Load File: %s", tex_file_name); }
		}
	}

	void set_texture(uint start_slot, ID3D11ShaderResourceView* tex) {
		...
		m_device_context->PSSetShaderResources(start_slot, 1, &tex); 
	}

		
	void set_texture_null(uint start_slot) {
		m_device_context->PSSetShaderResources(start_slot, 1, nullptr); 
	}
	...
	void create_sampler_state(ID3D11SamplerState*& sampler_state, rs_texture_filter_e filtering, int anisotropic_quality) {
	...
	void set_sampler_state(uint start_slot, ID3D11SamplerState* sampler_state) {
		m_device_context->PSSetSamplers(start_slot, 1, &sampler_state);
	}

	//-----------------------------
	void load_vertex_shader_file(const char* shader_filename, ID3DBlob*& shader_blob, ID3D11VertexShader*& vs_shader) {
		HRESULT hr = 0;
		bool success = false;
		success = d3d11_compile_shader_from_file(shader_filename, "VS", "vs_5_0", shader_blob);
	...
	void load_pixel_shader_file(const char* shader_filename, ID3DBlob*& shader_blob, ID3D11PixelShader*& ps_shader) {
		HRESULT hr = 0;
		bool success = false;
		success = d3d11_compile_shader_from_file(shader_filename, "PS", "ps_5_0", shader_blob);
	...
	//-----------------------------
	void create_model(d3d11_model& model, const void* vertices_data, isz vertices_size, uint vertices_stride, 
		const void* indices_data, isz indices_size, uint indices_stride, ID3D11InputLayout* input_layout, 
	...
	void create_model_basic(d3d11_model& model, const void* vertices_data, isz vertices_size, uint vertices_stride, 
		const void* indices_data, isz indices_size, uint indices_stride, ID3D11InputLayout* input_layout, 
		ID3D11ShaderResourceView* tex) {
	...
	template<class T>
	void create_model_rsmodel(d3d11_model& model, rs_model<T> rmodel, ID3D11InputLayout* input_layout, 
		ID3D11ShaderResourceView* tex) {
		create_model(model, rmodel.vertices_vec.data(), rmodel.vertices_vec.size(), rmodel.vstride(), 
				rmodel.indices_vec.data(), rmodel.indices_vec.size(), rmodel.istride(), input_layout, tex);	
	}	

	void draw_model(const d3d11_model& model) {
		set_texture(0, model.m_tex);		

		m_device_context->IASetInputLayout(model.m_input_layout);

		UINT offset = 0;
		m_device_context->IASetVertexBuffers(0, 1, &model.m_vb, &model.vstride, &offset);
        m_device_context->IASetIndexBuffer(model.m_ib, DXGI_FORMAT_R32_UINT, 0);		

		m_device_context->DrawIndexed(model.num_indices, 0, 0);	
	}


	//--------------------------------
	//void switch_to_fullscreen(bool fullscreen) {
	//	m_swap_chain->SetFullscreenState(fullscreen, nullptr);

	//	if (fullscreen) {
	//		resize(1920, 1080);
	//	} else {
	//		// store last window dimensions to use here
	//		resize(1280, 720);
	//	}
	//}	
...
public:		
	...
	#if 1
		const char* shader_filename = RS_SHADER_PATH2("d3d11_vertex_pc.fx");		
...
class rd3d11_tri_color_tex : public rd3d11  {
public:	

	d3d11_input_layout m_input_layout;
	...
	d3d11_vs_shader vs_shader;
    d3d11_blob vs_blob;
	d3d11_ps_shader ps_shader;
    d3d11_blob ps_blob;

	d3d11_buffer buffer_per_object;
	...
	void setup() {	
		...
		const char* shader_filename = RS_SHADER_PATH2("d3d11_vertex_pct.fx");		
		load_vertex_shader_file(shader_filename, vs_blob.data, vs_shader.data);
		load_pixel_shader_file(shader_filename, ps_blob.data, ps_shader.data);		
		...
		sdr::vector<D3D11_INPUT_ELEMENT_DESC> input_elements;
		d3d11_create_input_element_desc(sdf::span(vfmt_pc, sdf::arr_cap(vfmt_pc)), input_elements);
		// After vertex shader init.
		// Create the input layout.
		m_device->CreateInputLayout(input_elements.data(), input_elements.size(), vs_blob.data->GetBufferPointer(), 
			vs_blob.data->GetBufferSize(), &m_input_layout.data);				
	...
	void render(float dt) {
		...
		upload_constant_buffer(buffer_per_object.data, 1, &cb_per_object);

		set_sampler_state(0, m_sampler_anisotropic.data);

		// Set shaders and constant buffers
		m_device_context->VSSetShader(vs_shader.data, nullptr, 0);				
		m_device_context->PSSetShader(ps_shader.data, nullptr, 0);	
...
class rd3d11_cube : public rd3d11  {
public:	
	...
	void setup() {	
		...
		const char* shader_filename = RS_SHADER_PATH2("d3d11_vertex_ptn.fx");		
		load_vertex_shader_file(shader_filename, vs_blob.data, vs_shader.data);
		load_pixel_shader_file(shader_filename, ps_blob.data, ps_shader.data);		
		...
		create_model_rsmodel(model0, rmodel0, m_input_layout.data, tex0.data);

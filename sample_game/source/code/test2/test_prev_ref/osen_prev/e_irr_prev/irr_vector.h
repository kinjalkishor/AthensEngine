...
public:
	...
	*/
	void reallocate(uint new_size, bool canShrink=true)
	{
	...
	application. */
	void set_pointer(T* newPointer, uint size, bool _is_sorted=false, bool _free_when_destroyed=true)
	{
		clear();
		data = newPointer;
		allocated = size;
	...
	void erase(uint index, s32 count)
	{

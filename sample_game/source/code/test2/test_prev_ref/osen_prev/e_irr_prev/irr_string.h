...
static inline uint locale_lower ( uint x )
{
...
static inline uint locale_upper ( uint x )
{
	switch ( locale_current )
	{
		case IRR_LOCALE_GERMAN:
...
public:
	...
	template <class B>
	string(const B* const c, uint length)
	: array(0), allocated(0), used(0)
	{
		if (!c)
		{
	...
	bool equalsn(const string<T,TAlloc>& other, uint n) const
	{
		uint i;
	...
	string<T,TAlloc>& append(const T* const other, uint length=0xffffffff)
	{
		if (!other)
			return *this;
	...
	string<T,TAlloc>& append(const string<T,TAlloc>& other, uint length)
	{
		if (other.size() == 0)
			return *this;
	...
	or -1 if not found. */
	s32 findFirstChar(const T* const c, uint count=1) const
	{
		if (!c || !count)
			return -1;
	...
	or -1 if not found. */
	template <class B>
	s32 findFirstCharNotInList(const B* const c, uint count=1) const
	{
		if (!c || !count)
			return -1;
	...
	or -1 if not found. */
	template <class B>
	s32 findLastCharNotInList(const B* const c, uint count=1) const
	{
		if (!c || !count)
			return -1;
	...
	\return Position where the character has been found,
	or -1 if not found. */
	s32 findNext(T c, uint startPos) const
	{
		for (uint i=startPos; i<used-1; ++i)
			if (array[i] == c)
				return i;
	...
	\return Position where one of the characters has been found,
	or -1 if not found. */
	s32 findLastChar(const T* const c, uint count=1) const
	{
		if (!c || !count)
			return -1;
	...
	or -1 if not found. */
	template <class B>
	s32 find(const B* const str, const uint start = 0) const
	{
		if (str && *str)
		{
			uint len = 0;
	...
	string<T> subString(uint begin, s32 length, bool make_lower = false ) const
	{
	...
	string<T,TAlloc>& replace(const string<T,TAlloc>& toReplace, const string<T,TAlloc>& replaceWith)
	{
		...
		const uint other_size = toReplace.size();
		const uint replace_size = replaceWith.size();
	...
	string<T,TAlloc>& erase(uint index)
	{
	...
	*/
	...
	uint split(container& ret, const T* const c, uint count=1, bool ignoreEmptyTokens=true, bool keepSeparators=false) const
	{
		if (!c)
			return 0;

		const uint oldSize=ret.size();
		uint lastpos = 0;

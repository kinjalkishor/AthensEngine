...
public:
	...
	bool init(WNDPROC wnd_proc, HINSTANCE hInstance, const char* app_class_name) {
		sdf::string_st_wcs_from_mbs(m_app_class_name, app_class_name, sdf::strz_len(app_class_name));
	...
	bool create_window(const char* wnd_title, int xpos, int ypos, int width, int height, bool fullscreen, bool borderless) {
		...
		sdf::string_st_wcs_from_mbs(wstr_wnd_title, wnd_title, sdf::strz_len(wnd_title));
...
int WINAPI WinMain (HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR szCmdLine, int iCmdShow) {
	...
	int width = 800;
	int height = 450;
	const char* wnd_title = "Sample window q2";	
	...
	std::unique_ptr<rq2_gl2> rs;		
	rs = std::make_unique<rq2_gl2>();	

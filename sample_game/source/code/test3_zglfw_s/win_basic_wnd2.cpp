...
inline int mb_mbs_from_wcs(char* dest, int dest_num_chars, const wchar_t* src, int src_len) {
	const int dest_size_with_null_char = dest_num_chars+1;
...
inline void strf2_wcs_from_mbs(wchar_t* dest, isz& dest_len, isz dest_num_chars, const char* src, int src_len) {
    isz copy_len = sdf2::get_truncated_len(src_len, dest_num_chars);
	sdf2::mb_wcs_from_mbs(dest, dest_num_chars, src, copy_len);
	dest[copy_len] = k_null_char<wchar_t>();
	dest_len = copy_len;
}
inline void strf2_mbs_from_wcs(char* dest, isz& dest_len, isz dest_num_chars, const wchar_t* src, int src_len) {
    isz copy_len = sdf2::get_truncated_len(src_len, dest_num_chars);
	sdf2::mb_mbs_from_wcs(dest, dest_num_chars, src, copy_len);
...
public:
	...
	bool create_window(const char* window_title, int xpos, int ypos, int width, int height) {
		...
		isz wconsole_title_size = 0;
		sdf2::strf2_wcs_from_mbs(wconsole_title, wconsole_title_size, sdf2::strz_cap(wconsole_title), window_title, sdf2::strz_len(window_title));
...
bool winapp_create_window(HWND& m_hwnd, const char* wnd_title, int xpos, int ypos, int width, int height, 
	...
	isz wstr_wnd_title_size = 0;
	sdf2::strf2_wcs_from_mbs(wstr_wnd_title, wstr_wnd_title_size, sdf2::strz_cap(wstr_wnd_title), wnd_title, sdf2::strz_len(wnd_title));
